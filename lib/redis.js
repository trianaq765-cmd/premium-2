...
async function isBanned(hwid, ip, playerId) {
    ...
    try {
        // Hanya cek HWID dan playerId, BUKAN IP
        const checks = [hwid, String(playerId)].filter(Boolean);
...
async function setChallenge(id, data, ttl = 120) {  // Ubah dari 60 ke 120 detik
    if (!redis) return false;
    try {
...
async function clearBans(){
    const keys = await client.keys('ban:*');
    if(keys.length > 0){
...
module.exports = {
    addBan, removeBan, isBanned, clearBans, getAllBans,
    addLog, getLogs,
