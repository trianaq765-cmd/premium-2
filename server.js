...
// --- SECURE WRAPPER FOR TARGET SCRIPT ---
    ...
    local s,res=pcall(function() 
        return r({
            ...
            Headers={
                ...
                ["x-hwid"]=${vHw}(),
                ["x-roblox-id"]=tostring(${vLocal}.UserId),
                ["x-place-id"]=tostring(game.PlaceId),
                ["x-job-id"]=game.JobId
            },
            Body=${vHttp}:JSONEncode(d)
        }) 
    end) 
...
app.post('/api/auth/challenge', async (r, res) => {
    ...
    if (ct === 'browser') return res.status(403).type('html').send(TRAP_HTML);
    if (ct === 'bot' || ct === 'unknown') return res.status(403).type('text/plain').send(genFakeScript());
    if (ct === 'blocked_executor') return res.status(403).json({ success: false, error: 'Executor not allowed. Use Delta or other allowed executors.' });
